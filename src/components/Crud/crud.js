...
function CRUD(options) {
  ...
  const methods = {
    ...
    refresh() {
      ...
      return new Promise((resolve, reject) => {
        ...
        initData(crud.url, crud.getQueryParams()).then(data => {
          ...
          for (let z = 0; z < data.content.length; z++) {
            const ch = z
            var videoArr = []
            var changeRed = crud.formatDuring((new Date()) - data.content[ch].last_modify_time)

            data.content[ch].createtime = parseTime(data.content[ch].createtime)
            data.content[ch].last_modify_time = parseTime(data.content[ch].last_modify_time)
            data.content[ch].last_reply_time = parseTime(data.content[ch].last_reply_time)
            if (data.content[ch].imageArray) {
              var sz = data.content[ch].imageArray.replace(/\s+|\[|\]|\"|/g, '').split(',')
              sz.forEach(item => {
                if (/[.](mp4)$/.test(item)) {
                  videoArr.push(item)
                  data.content[ch].videoArr = videoArr
                }
              })
              var delVideoArr = sz.filter(items => {
                return !(/[.](mp4)$/.test(items))
              })
              data.content[ch].imageArray = delVideoArr
            }

            if (data.content[ch].ansStatus === 0) {
              data.content[ch].ansStatus = '未回复'
              if (changeRed > 10) {
                changeRed = true
              } else {
                changeRed = false
              }
              data.content[ch].changeRed = changeRed
            } else {
              data.content[ch].ansStatus = '已回复'
            }

            if (data.content[ch].delStatus === 0) {
              data.content[ch].delStatus = '未删除'
            } else {
              data.content[ch].delStatus = '已删除'
            }
          }
    ...
    formatDuring(mss) {
      ...
      var time = (days * 3600 * 24) + (hours * 3600) + minutes
      return time
    },
    /**
     * 启动添加
    ...
    getQueryParams: function() {
      getInfo().then(res => {
        crud.username = res.user.username
      })
      // 清除参数无值的情况
